...
class TfGraphDataViewProviderSpec(ComponentSpec):
...
class TfGraphDataViewProvider(base_component.BaseComponent):
  ...
  The use defines the a function that creates such a TfGraphRecordDecoder. This
  component, when running, calls that function and writes the result decoder
  as its output artifact.
  ...
  SPEC_CLASS = TfGraphDataViewProviderSpec
  ...
  def __init__(self,
    ...
    spec = TfGraphDataViewProviderSpec(
        module_file=module_file,
