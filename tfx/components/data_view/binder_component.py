...
class DataViewBinderComponentSpec(ComponentSpec):
...
class DataViewBinder(base_component.BaseComponent):
  ...
  ```
  """
  SPEC_CLASS = DataViewBinderComponentSpec
  ...
  def __init__(self,
    ...
    if not output_examples:
      output_examples = channel_utils.as_channel(
          [_forward_examples(ea) for ea in input_examples.get()])

    spec = DataViewBinderComponentSpec(
...
def _forward_examples(
    artifact: standard_artifacts.Examples) -> standard_artifacts.Examples:
  result = standard_artifacts.Examples()
  result.span = artifact.span
  result.split_names = artifact.split_names
  return result
