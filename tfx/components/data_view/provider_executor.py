...
CREATE_DECODER_FUNC_KEY = 'create_decoder_func'
  ...
  def Do(self, input_dict: Dict[Text, List[types.Artifact]],
    ...
    if MODULE_FILE_KEY in exec_properties:
      create_decoder_func = import_utils.import_func_from_source(
          exec_properties.get(MODULE_FILE_KEY),
          exec_properties.get(CREATE_DECODER_FUNC_KEY))
    else:
      create_decoder_func = udf_utils.get_fn(
          exec_properties, CREATE_DECODER_FUNC_KEY)
    tf_graph_record_decoder.save_decoder(
        create_decoder_func(),
        value_utils.GetSoleValue(output_dict, DATA_VIEW_KEY).uri)
